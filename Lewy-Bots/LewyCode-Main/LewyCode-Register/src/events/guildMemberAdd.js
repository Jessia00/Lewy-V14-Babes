...
module.exports = async (member) => {
  ...
  const data = await forceBans.findOne({ guildID: lewyim.GuildID, userID: member.user.id });
 ...
 const tagModedata = await regstats.findOne({ guildID: lewyim.GuildID })
  if (tagModedata && tagModedata.tagMode === false) {
    if (otoreg) {
     ...
     await isimler.findOneAndUpdate({ guildID: lewyim.GuildID, userID: member.user.id }, { $push: { names: { name: member.displayName, sebep: "Oto.Bot Kayıt", rol: otoreg.roleID.map(x => `<@&${x}>`), date: Date.now() } } }, { upsert: true });
    }
}
  ...
  const res = await bannedTag.findOne({ guildID: lewyim.GuildID });
  if (!res) return
  ...
  if(res.taglar.some(x => member.user.tag.includes(x))) { 
    ...
    if (lewyim.Main.dmMessages) member.send({ content:`${member.guild.name} adlı sunucumuza olan erişiminiz engellendi! Sunucumuzda yasaklı olan bir simgeyi (${x}) isminizde taşımanızdan dolayıdır. Sunucuya erişim sağlamak için simgeyi (${x}) isminizden çıkartmanız gerekmektedir.\n\nSimgeyi (${x}) isminizden kaldırmanıza rağmen üstünüzde halen Yasaklı Tag rolü varsa sunucudan gir çık yapabilirsiniz veya sağ tarafta bulunan yetkililer ile iletişim kurabilirsiniz. **-Yönetim**\n\n__Sunucu Tagımız__\n**${conf.tag}**`}).catch(() => {});
}
}) 
