...
module.exports = async (message) => {
  ...
  if (message.author.bot) return;
    ...
    await client.checkLevel(message.author.id, lewyim.GuildID, "mesaj")
	let data = await Stat.findOne({ userID: message.author.id, guildID: lewyim.GuildID }) || { messageLevel: 0 };
	...
	await Stat.findOneAndUpdate({ guildID: lewyim.GuildID, userID: message.author.id }, { $set: {["messageXP"]: 0}, $inc: {["messageLevel"]: 1 }}, { upsert: true });
	await lewy.findOneAndUpdate({ guildID: lewyim.GuildID, userID: message.author.id }, { $inc: { dolar: +para } }, { upsert: true });
...
await Stat.findOneAndUpdate({ guildID: lewyim.GuildID, userID: id }, { $inc: { messageXP: randomMessageXP } }, { upsert: true });
};
