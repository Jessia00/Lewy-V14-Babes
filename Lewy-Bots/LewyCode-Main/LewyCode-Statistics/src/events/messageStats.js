...
module.exports = async (message) => {
  if (message.author.bot || !message.guild || message.content.startsWith(lewyim.Main.prefix)) return;
  ...
  if (conf.staffs.some(x => message.member.roles.cache.has(x)) && !conf.sahipRolu.some(x => message.member.roles.cache.has(x))) {
    ...
    if (num && (num % lewyim.Main.messageCount) === 0) {
      nums.set(message.author.id, num + 1);
      await coin.findOneAndUpdate({ guildID: message.guild.id, userID: message.author.id }, { $inc: { coin: lewyim.Main.messageCoin } }, { upsert: true });
  ...
  await dolar.findOneAndUpdate({ guildID: message.guild.id, userID: message.author.id }, { $inc: { dolar: lewyim.Main.messageDolar } }, { upsert: true });
  }
