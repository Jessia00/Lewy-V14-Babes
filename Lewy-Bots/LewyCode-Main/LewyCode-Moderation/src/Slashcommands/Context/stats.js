...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        const member = await client.guilds.cache.get(lewyim.GuildID).members.fetch(interaction.targetId)
    ...
    const ctx = canvas.getContext('2d');
     ...
     let lewy = Math.floor((Math.random() * Jaylen.length));
    
        const background = await Canvas.loadImage(Jaylen[lewy]);  
        ctx.save();
