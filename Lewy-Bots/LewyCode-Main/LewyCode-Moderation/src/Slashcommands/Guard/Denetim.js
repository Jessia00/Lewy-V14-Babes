...
const CategoryChannels = require("../../../../../LewyCode-Guard/src/Models/CategoryChannels");
const TextChannels = require("../../../../../LewyCode-Guard/src/Models/TextChannels");
const VoiceChannels = require("../../../../../LewyCode-Guard/src/Models/VoiceChannels");
const RoleModel = require("../../../../../LewyCode-Guard/src/Models/Role");
const lewyim = require("../../../../../../config.json");
const moment = require("moment");
require("moment-duration-format");
const client = global.bot;
const Bots = require("../../../../../LewyCode-Guard/Backup")
  ...
  async execute(interaction, bot) {
   if(!lewyim.owners.includes(interaction.user.id)) {
      return interaction.reply({ content: ":x: Bot developerı olmadığın için kullanamazsın.", ephemeral: true })
       ...
       const lewy = interaction.options.getString("kategori");
       const guild = client.guilds.cache.get(lewyim.GuildID);
        ...
        if (lewy === "roles") {
          ...
          interaction.reply({ content: `Son 1 saat içinde **${denetim.length || 0}** adet rol silinmiş.`, components: [row], ephemeral: true })        
            ...
            if(kurulacakRole) {
            const RoleData = await RoleModel.findOne({ guildID: lewyim.GuildID, roleID: kurulacakRole });
...
İşlemin biteceği tahmini süre: ${(length>1000 ? parseInt((length*(lewyim.Guard.Guard.GiveRoleDelay/1000)) / 60)+" dakika" : parseInt(length*(lewyim.Guard.Guard.GiveRoleDelay/1000))+" saniye")}
\`\`\``, ephemeral: true })
                     ...
                     for (let index = 0; index < availableBots.length; index++) {
                         ...
                         let guild = bot.guilds.cache.get(lewyim.GuildID); 
                         ids.every(async id => {
