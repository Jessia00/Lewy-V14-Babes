...
module.exports = async (message) => {
  let prefix = lewyim.Main.prefix.find((x) => message.content.toLowerCase().startsWith(x));
  ...
  const embed = new EmbedBuilder().setFooter({ text: lewyim.AltBaşlık}).setAuthor({ name: message.member.displayName, iconURL: message.author.avatarURL({ dynamic: true })});
  ...
  if (cmd) {
    if (cmd.conf.owner && !lewyim.owners.includes(message.author.id)) return;
    ...
    `)
    if(komutLog) komutLog.wsend({ embeds: [lewy]})
  }
};
