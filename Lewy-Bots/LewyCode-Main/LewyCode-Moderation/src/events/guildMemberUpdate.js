    ...
    newMember.roles.cache.forEach(async role => {
      if (!oldMember.roles.cache.has(role.id)) {
        const emed = new EmbedBuilder()
          ...
          .setDescription(`Kişinin eklenen ve alınan tüm rollerine bakmak için \`!rollog @lewy\` komutunu kullanın \n**Rol Eklenen kişi**\n ${hedef} - **${hedef.id}** `)
          .addFields(
    ...
    oldMember.roles.cache.forEach(async role => {
      if (!newMember.roles.cache.has(role.id)) {
        ...
        .setAuthor({ name: hedef.tag, iconURL: hedef.displayAvatarURL({ dynamic: true }) })
          ...
          .setDescription(`Kişinin alınan ve eklenen tüm rollerine bakmak için \`!rollog @lewy\` komutunu kullanın \n**Rolü Alınan kişi** \n${hedef} - **${hedef.id}**`)
          .addFields(
