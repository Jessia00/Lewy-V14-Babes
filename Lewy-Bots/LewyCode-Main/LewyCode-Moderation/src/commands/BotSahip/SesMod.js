...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    const data = await Database.findOne({ guildID: lewyim.GuildID });
...
collector.on("collect", async (menu) => {
  await menu.deferUpdate();
    if(menu.values[0] === "mod") {
      ...
      await Database.findOneAndUpdate({ guildID: lewyim.GuildID }, {SesMod: hosgeldin}, { upsert: true })
      msg.edit({ content: `**Ses modu** başarıyla \` Hoş geldin \` olarak ayarlandı.`, components: [] });
      children.exec(`pm2 restart ${lewyim.GuildName}_Welcomes`);
    }
    if(menu.values[0] === "mod2") {
      ...
      await Database.findOneAndUpdate({ guildID: lewyim.GuildID }, {SesMod: taglialim}, { upsert: true })
    ...
    if(menu.values[0] === "mod3") {
      ...
      await Database.findOneAndUpdate({ guildID: lewyim.GuildID }, {SesMod: konser}, { upsert: true })
    ...
    if(menu.values[0] === "mod4") {
      ...
      await Database.findOneAndUpdate({ guildID: lewyim.GuildID }, {SesMod: turnuva}, { upsert: true })
    ...
    if(menu.values[0] === "mod5") {
      ...
      await Database.findOneAndUpdate({ guildID: lewyim.GuildID }, {SesMod: toplantı}, { upsert: true })
    ...
    if(menu.values[0] === "mod6") {
      ...
      await Database.findOneAndUpdate({ guildID: lewyim.GuildID }, {SesMod: cekilis}, { upsert: true })
