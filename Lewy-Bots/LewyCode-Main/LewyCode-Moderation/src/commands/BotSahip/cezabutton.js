...
client.on('interactionCreate', async interaction => {
  ...
  const cezaData = await ceza.findOne({ guildID: lewyim.GuildID, userID: member.id });
  const cezapuanData = await cezapuan.findOne({ guildID: lewyim.GuildID, userID: member.id });
 ...
 Toplamda \`${cezapuanData ? cezapuanData.cezapuan : 0} ceza puanı\` ve (Toplam **${cezaData ? cezaData.ceza.length : 0}** Ceza) olarak gözükmekte.`, ephemeral: true
  ...
  let data = await penals.find({ guildID: lewyim.GuildID, userID: member.id, active: true }).sort({ date: -1 });
  ...
  let datas = await penals.find({ guildID: lewyim.GuildID, userID: member.id, active: true }).sort({ date: -1 });
