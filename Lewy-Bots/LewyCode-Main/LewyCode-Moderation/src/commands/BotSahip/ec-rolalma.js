...
const { lewynitro, lewynetflix, lewyspotify, lewyexxen, lewyblutv} = require("../../../../src/configs/emojis.json")
...
const etkinlik = await client.guilds.cache.get(lewyim.GuildID).roles.cache.find(x => x.name.includes(conf.etkinlik))
const cekilis = await client.guilds.cache.get(lewyim.GuildID).roles.cache.find(x => x.name.includes(conf.cekilis))
