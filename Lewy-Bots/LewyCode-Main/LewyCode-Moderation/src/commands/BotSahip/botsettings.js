...
run: async (client, message, args, embed) => {
    ...
    const mesaj = await message.channel.send({ embeds: [embed.setDescription(`${green} Aşağıda sıralanmakta olan botların, ismini veya profil fotoğrafını değiştirmek istediğinz botu seçin.`)], components: [row, row2] });
    ...
    collector.on("collect", async (menu) => {
        ...
        if (menu.customId == "allres") {
            ...
            if(lewyim.Destek.Active) {
                children.exec(`pm2 restart ${lewyim.GuildName}_Destek`);
            }
