...
client.on('interactionCreate', async interaction => {
    if(interaction.customId === "create") {
        ...
        const xd = await client.guilds.cache.get(lewyim.GuildID).channels.cache.get(data?.ID)
...
const Channel = Guild?.channels.cache.get(room?.ID);
    ...
    return }
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(lewyim.GuildID, { Connect: false });
        ...
        await interaction.reply({ content: `Kanal sahibi olmadığınız için bu butonu kullanamazsınız.`, ephemeral: true })
        return }    
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(lewyim.GuildID, { Connect: true });
        ...
        await interaction.reply({ content: `Kanal sahibi olmadığınız için bu butonu kullanamazsınız.`, ephemeral: true })
        return }    
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(lewyim.GuildID, { ViewChannel: false });
        ...
        await interaction.reply({ content: `Kanal sahibi olmadığınız için bu butonu kullanamazsınız.`, ephemeral: true })
        return }    
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(lewyim.GuildID, { ViewChannel: true });
        ...
        let msg = await interaction.channel.send({ content: `<@${rooms.Owner}>`, embeds: [prolewy], components: [pro] })
        ...
        if (button.customId === "onays") {
            ...
            .setAuthor({ name: client.guilds.cache.get(lewyim.GuildID).name, iconURL: client.guilds.cache.get(lewyim.GuildID).iconURL({ dynamic: true, size: 2048 })})
    ...
    collector.on('end', i => {
       msg.delete();
        ...
        await interaction.reply({ content: `Lütfen kanalınıza girmesini istediğiniz üye(leri) etiketleyiniz. \`Örn: @lewy\``, ephemeral: true })
       ...
       let yarak = await interaction.channel.send({ content: `Odaya erişimini kaldırmak istediğiniz üyeleri listeden seçiniz.`, components: [row]});
            ...
            yarak.awaitMessageComponent({ filter, componentType: ComponentType.StringSelect, time: 60000 }).then((x) => {
                x.values.map(async (v) => {
                    ...
                    let memb = client.guilds.cache.get(lewyim.GuildID).members.cache.get(v)
        ...
        await interaction.reply({ content: `Kanal sahipliğini aktarmak istediğiniz üyeyi etiketleyiniz. \`Örn: @lewy\``, ephemeral: true })
        interaction.channel.awaitMessages({ filter, max: 1 }).then(async function (collected) {
...
client.on("ready", async () => {
    const sunucu = client.guilds.cache.get(lewyim.GuildID);
