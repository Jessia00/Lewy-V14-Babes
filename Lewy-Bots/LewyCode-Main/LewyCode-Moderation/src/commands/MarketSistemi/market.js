...
.addFields(
  { name: `${star} Mağaza (\`Bakiye: ${dolarData ? Math.floor(parseInt(dolarData.dolar)) : 0} 💵\`)`,  value: `\`\`\`css
  ${table.table(urunler, {
...
{ name: `${star} Ürün nasıl satın alabilirim?`,  value: `Aşağıda beliren butonlardan yeşil olanlara \`30 Saniye\` içerisinde tıklayarak satın alabilirsin.`, inline: false },
  )
   
let Jaylen = await message.channel.send({ embeds: [lewy],  components: [market, market2] });
    ...
    if (button.customId === "lewyspotify") {
...
.setThumbnail(message.author.displayAvatarURL({ dynamic: true, size: 2048 }))
        ...
        client.channels.cache.find(x => x.name == "market_log").send(`${message.author} kişisi \`Spotify Premium\` ürününü satın aldı. İletişime geçmenizi bekliyor! :tada:`)
         await dolar.findOneAndUpdate({ guildID: lewyim.GuildID, userID: message.author.id }, { $inc: { dolar: -40000 } }, { upsert: true });
         
      Jaylen.edit({ embeds: [spotify], components: []}); 

        }

      if (button.customId === "lewynetflix") {
        await button.deferUpdate();
...
client.channels.cache.find(x => x.name == "market_log").send(`${message.author} kişisi \`Netflix UHD\` ürününü satın aldı. İletişime geçmenizi bekliyor! :tada:`)
         await dolar.findOneAndUpdate({ guildID: lewyim.GuildID, userID: message.author.id }, { $inc: { dolar: -50000 } }, { upsert: true });
         
      Jaylen.edit({ embeds: [netflix], components: []}); 

        }

      if (button.customId === "lewyyoutube") {
        await button.deferUpdate();
...
client.channels.cache.find(x => x.name == "market_log").send(`${message.author} kişisi \`Youtube Premium\` ürününü satın aldı. İletişime geçmenizi bekliyor! :tada:`)
         await dolar.findOneAndUpdate({ guildID: lewyim.GuildID, userID: message.author.id }, { $inc: { dolar: -60000 } }, { upsert: true });

      Jaylen.edit({ embeds: [youtube], components: []}); 

        }

       if (button.customId === "lewycnitro") {
        await button.deferUpdate();
...
client.channels.cache.find(x => x.name == "market_log").send(`${message.author} kişisi \`Classic Nitro\` ürününü satın aldı. İletişime geçmenizi bekliyor! :tada:`)
         await dolar.findOneAndUpdate({ guildID: lewyim.GuildID, userID: message.author.id }, { $inc: { dolar: -125000 } }, { upsert: true });

      Jaylen.edit({ embeds: [cnitro], components: []}); 

        }

      if (button.customId === "lewybnitro") {
        await button.deferUpdate();
...
client.channels.cache.find(x => x.name == "market_log").send(`${message.author} kişisi \`Boostlu Nitro\` ürününü satın aldı. İletişime geçmenizi bekliyor! :tada:`)
         await dolar.findOneAndUpdate({ guildID: lewyim.GuildID, userID: message.author.id }, { $inc: { dolar: -150000 } }, { upsert: true });
