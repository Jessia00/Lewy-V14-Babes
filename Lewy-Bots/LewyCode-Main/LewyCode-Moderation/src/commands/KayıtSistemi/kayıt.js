...
Üyesinin önceki isimlerine \`.isimler <@lewy/ID>\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.
`)
...
if(button.customId === "MAN") {
  ...
  if(msg) msg.delete();
  button.reply({ embeds: [lewye], components: [], ephemeral: true});

    await uye.roles.add(ayar.erkekRolleri)
    await uye.roles.remove(ayar.unregRoles)
    await coin.findOneAndUpdate({ guildID: uye.guild.id, userID: message.author.id }, { $inc: { coin: lewyim.Main.toplamsCoin } }, { upsert: true });
...
if(msg) msg.delete();
button.reply({ embeds: [lewyk], components: [], ephemeral: true});
