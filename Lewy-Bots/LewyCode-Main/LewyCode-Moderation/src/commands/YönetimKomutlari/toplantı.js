...
**${guild.name}** sunucusunda şuan da toplantı durumu: **${meetingStatus ? "Aktif" : "Devre-dışı"}** ${meetingStatus ? green : red}`)
        ...
        collector.on('collect', async (i) => {
          if(i.customId == "toplantıAç") {
              let meetingStatus = MeetingData.Toplantı || false
              if(meetingStatus) {
                await meetings.updateOne({guildID: lewyim.GuildID}, {$set: {Toplantı: false}}, {upsert: true})
...
> Bu toplantı  **\`${meetingTime(KatilimData.endDate - KatilimData.Date)}\`** sürmüş.`)], components: []})
                ...
                let datax = await meetings.findOne({ guildID: lewyim.GuildID })
                if(!datax) {await meetings.updateOne({guildID: lewyim.GuildID}, {$set: {Toplantı: true}}, {upsert: true})}
                await meetings.updateOne({guildID: message.guild.id}, {$set: {
                  "Date": Date.now(),
                  "userID": i.user.id,
                ...
                let datax2 = await meetings.findOne({ guildID: lewyim.GuildID })
                if (datax2) {
