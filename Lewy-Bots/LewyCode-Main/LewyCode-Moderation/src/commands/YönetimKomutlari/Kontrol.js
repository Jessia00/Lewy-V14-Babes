...
module.exports = {
    ...
    return }
       ...
       const etkinlik = await client.guilds.cache.get(lewyim.GuildID).roles.cache.find(x => x.name.includes(conf.etkinlik)).id
       const cekilis = await client.guilds.cache.get(lewyim.GuildID).roles.cache.find(x => x.name.includes(conf.cekilis)).id
...
.setThumbnail(message.author.displayAvatarURL({ dynamic: true, size: 2048 }))
  ...
  let msg = await message.channel.send({ embeds: [lewy], components: [row]})
    ...
    let lewy = message.guild.members.cache.filter(member => !member.roles.cache.has(etkinlik) || !member.roles.cache.has(cekilis))
    button.reply({ content:`
Etkinlik/Çekiliş rolü olmayan ${lewy.size} kullanıcıya etkinlik, çekiliş rolleri verildi !`})
...
${taglilar.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")}`})
    ...
    button.reply({ content:`
Kayıtsız rolü olmayan ${lewycim.size} kullanıcıya kayıtsız rolü verildi !

Kayıtsız Rolü verilen kullanıcılar;
${lewycim.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")} `})
