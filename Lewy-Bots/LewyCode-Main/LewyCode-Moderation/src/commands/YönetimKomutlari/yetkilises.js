...
module.exports = {
  ...
  if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator)) return message.react(red)
    ...
    if (sec) {
      if(!client.guilds.cache.get(lewyim.GuildID).roles.cache.get(sec)) {
      ...
      let lewy = message.guild.members.cache.filter(yetkili => yetkili.roles.cache.has(sec)).filter(yetkilises => !yetkilises.voice.channel  && yetkilises.presence && yetkilises.presence.status != "offline")
      ...
      let lewy = message.guild.members.cache.filter(yetkili => yetkili.roles.cache.has(conf.staffs[0])).filter(yetkilises => !yetkilises.voice.channel  && yetkilises.presence && yetkilises.presence.status != "offline")
