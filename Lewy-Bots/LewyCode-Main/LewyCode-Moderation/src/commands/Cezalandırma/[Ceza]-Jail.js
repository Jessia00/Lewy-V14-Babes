...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    {
    message.react(red)
    message.channel.send({ content:"Yeterli yetkin bulunmuyor lewy'a ulaş !"}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
    ...
    if (lewyim.Main.jaillimit > 0 && jailLimit.has(message.author.id) && jailLimit.get(message.author.id) == lewyim.Main.jaillimit) 
    {
    message.react(red)
...
if (duration) {
  ...
  if (lewyim.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar jaillendiniz.`}).catch(() => {});
