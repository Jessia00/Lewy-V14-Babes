...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    {
    message.react(red)
    message.channel.send({ content:"Yeterli yetkin bulunmuyor lewy ile görüş !"}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
    ...
    if (button.customId === "dosya") {
      ...
      button.reply({ content: `${member} kullanıcısının toplamda **${cezaData ? cezaData.ceza.length : 0}** cezası  bulunmaktadır ve aşağıdaki belgede yazmaktadır.`, files: [{ attachment: Buffer.from(lewy), name: `${member.user.username}_sicil.txt` }], components: [], ephemeral: true }); 
