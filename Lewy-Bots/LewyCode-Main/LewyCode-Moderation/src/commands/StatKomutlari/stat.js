...
run: async (client, message, args, prefix) => {
  ...
  if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && !kanallar.includes(message.channel.name)) return message.reply({ content: `${kanallar.map(x => `${client.channels.cache.find(chan => chan.name == x)}`)} kanallarında kullanabilirsiniz.`}).then((e) => setTimeout(() => { e.delete(); }, 10000)); 
    ...
    await client.guilds.cache.get(lewyim.GuildID).members.fetch(member.user.id)
    ...
    const category = async (parentsArray) => {
      ...
      let messageTop2;
      Active3.length > 0 ? messageTop2 = Active3.splice(0, 3).map(x => client.guilds.cache.get(lewyim.GuildID).channels.cache.get(x.channelID) ? `#${client.guilds.cache.get(lewyim.GuildID).channels.cache.get(x.channelID).name}` : `Kanal Bulunamadı`).join("\n\n\n") : messageTop2 = ""

      let messageTop3;
      Active4.length > 0 ? messageTop3 = Active4.splice(0, 3).map(x => `${Number(x.channelData).toLocaleString()} mesaj`).join("\n\n\n") : messageTop3 = ""

      let voiceTop;
      Active2.length > 0 ? voiceTop = Active2.splice(0, 3).map(x => client.guilds.cache.get(lewyim.GuildID).channels.cache.get(x.channelID) ? client.guilds.cache.get(lewyim.GuildID).channels.cache.get(x.channelID).name : `Kanal Bulunamadı`).join("\n\n\n") : voiceTop = ""
...
new ButtonBuilder().setCustomId("sonra").setLabel("Sonraki Sayfa").setStyle(ButtonStyle.Success).setEmoji("⏭️"),
  ...
  let page = 1;
  let liste = Active8.map((x, index) => `\` ${index+1} \` ${client.guilds.cache.get(lewyim.GuildID).channels.cache.get(x.channelID) ? client.guilds.cache.get(lewyim.GuildID).channels.cache.get(x.channelID).name : `Kanal Bulunamadı`}: \`${moment.duration(x.channelData).format("H [saat], m [dakika] s [saniye]")}\``)
...
msg.edit({ embeds: [embeds], components : [row, row2], files: [] })
  ...
  const Active6 = await streamerUserChannel.find({ guildID: message.guild.id, userID: member.user.id }).sort({ channelData: -1 });
    ...
    let page = 1;
    let liste = Active6.map((x, index) => `\` ${index+1} \` ${client.guilds.cache.get(lewyim.GuildID).channels.cache.get(x.channelID) ? client.guilds.cache.get(lewyim.GuildID).channels.cache.get(x.channelID).name : `Kanal Bulunamadı`}: \`${moment.duration(x.channelData).format("H [saat], m [dakika] s [saniye]")}\``)
...
if(interaction.values[0] === "stat4") {
  ...
  const Active7 = await cameraUserChannel.find({ guildID: message.guild.id, userID: member.user.id }).sort({ channelData: -1 });
    ...
    let page = 1;
    let liste = Active7.map((x, index) => `\` ${index+1} \` ${client.guilds.cache.get(lewyim.GuildID).channels.cache.get(x.channelID) ? client.guilds.cache.get(lewyim.GuildID).channels.cache.get(x.channelID).name : `Kanal Bulunamadı`}: \`${moment.duration(x.channelData).format("H [saat], m [dakika] s [saniye]")}\``)
