...
const applyText = (canvas, text) => {
      ...
      let background = await Canvas.loadImage(client.guilds.cache.get(lewyim.GuildID).bannerURL({dynamic: true, extension: "png"}) ? client.guilds.cache.get(lewyim.GuildID).bannerURL({dynamic: true, extension: "png"}) + `?size=4096` : "https://cdn.discordapp.com/attachments/938786568175513660/970470122806472785/sex.png");
...
if(random >= 98) shipdurum = "💖💖💖💖💖💖💖💖💖💖";
    ...
    if (random >= 60) {
      ...
      const attachment = new AttachmentBuilder(canvas.toBuffer(), { name: 'lewy.png'});
...
${shipdurum}`)
.setImage("attachment://lewy.png")
message.channel.send({ content: `**[ \`${member.user.tag}\` ]**`, embeds: [lewy], files: [attachment] })
} else if (random <= 40) {
