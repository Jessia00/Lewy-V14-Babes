...
client.on('ready', async () => {
  ...
  setInterval(async () => {
  let sesKanal = client.channels.cache.get(lewyim.BotSesKanal);
  if (sesKanal) { joinVoiceChannel({
      channelId: lewyim.BotSesKanal,
      guildId: guild.id,
  ...
  let activities = lewyim.Destek.Activity, i = 0;
...
const rest = new REST({ version: '10' }).setToken(lewyim.Destek.BotToken);

(async () => {
	try {
		console.log('[lewy] Komutlar yükleniyor.');

    await rest.put(
      Routes.applicationCommands(lewyim.Destek.BotClientID),
      { body: commands },
    );

		console.log('[lewy] Komutlar yüklendi.');
	} catch (error) {
		console.error(error);
	}
})();
...
client.on('modalSubmit', async (modal) => {
  ...
  var LogChannel = client.guilds.cache.get(lewyim.GuildID).channels.cache.find((channel) => channel.id === settings.BaşvuruLogChannelID);
    if(modal.customId === 'ybasvuru') {
      ...
      if (hakkında) {
  let lewy = new Discord.EmbedBuilder()
  .setDescription(`
  **${modal.user.tag}** - (\`${modal.user.id}\`) **Kullanıcısının Başvuru Formu**
  ...
  .setTimestamp()
        ...
        await LogChannel.send({ content: `<@&${conf.yetkilialımRol}> ${modal.user}`,  embeds: [lewy] })      
      }
    }  
  });

  client
  .login((lewyim.Destek.BotToken))
