...
const { green , red } = require('../../LewyCode-Main/src/configs/emojis.json');
...
.setAuthor({
   ...
   var LogChannel = client.guilds.cache.get(lewyim.GuildID).channels.cache.find((channel) => channel.id === settings.ÖneriİstekSikayetChannelID);
   LogChannel.send({ embeds: [lewy] })
})
...
var LogChannel = client.guilds.cache.get(lewyim.GuildID).channels.cache.find((channel) => channel.id === settings.CanlıDestekLogChannelID);
...
`)
.setAuthor({ name: "Canlı Destek", iconURL: client.guilds.cache.get(lewyim.GuildID).iconURL({ dynamic: true, size: 2048 }) })
.setThumbnail(interaction.user.displayAvatarURL({ dynamic: true, size: 2048 }))
.setTimestamp()

let msg = await LogChannel.send({ content: `<@&${conf.canlıdestekRol}>`, embeds: [lewy], components: [row3] });
...
components : []
  ...
  client.guilds.cache.get(lewyim.GuildID).channels.create({ name: `${interaction.user.username}-destek`,
    ...
    permissionOverwrites: [{
      ...
      {
        id: client.guilds.cache.get(lewyim.GuildID).roles.everyone,
...
if(interaction.customId === "basvuru") {
  ...
  if(!client.guilds.cache.get(lewyim.GuildID).members.cache.get(interaction.user.id).roles.cache.has(conf.ekipRolu)) {
    return interaction.reply({ content: ":x: Tagın olmadığı için başvuramazsın.", ephemeral: true })
  } else if([conf.staffs].some(role2 => client.guilds.cache.get(lewyim.GuildID).members.cache.get(interaction.user.id).roles.cache.get(role2))) {
    ...
    .addComponents([
      ...
      .setMinLength(5)
      .setMaxLength(20)
      .setPlaceholder('Lütfen buraya yazın. / Örn: lewy 18')
      .setRequired(true),
      new TextInputBuilder()
