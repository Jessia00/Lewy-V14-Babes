let { lewy } = require('./Voices.Global.Client');
const { ActivityType } = require('discord.js');
const { joinVoiceChannel, getVoiceConnection } = require('@discordjs/voice');
const lewyim = require('./../../config.json');
require("./src/handlers/mongoHandler");

for (let index = 0; index < lewyim.Welcome.Tokens.length; index++) {
    let token = lewyim.Welcome.Tokens[index]
    let channel = lewyim.Welcome.Channels < 1 ? lewyim.Welcome.Channels[0] : lewyim.Welcome.Channels[index]
    if(channel) {
        let client = new lewy()
            ...
            cron.schedule('* * * * 5', () => {
                ...
                if (day === 5) {
                    client.guilds.cache.forEach(async (guild) => {
                        ...
                        await Database.findOneAndUpdate({ guildID: lewyim.GuildID }, {SesMod: xd}, { upsert: true })
                    ...
                    client.guilds.cache.forEach(async (guild) => {
                        ...
                        await Database.findOneAndUpdate({ guildID: lewyim.GuildID }, {SesMod: xd2}, { upsert: true })
